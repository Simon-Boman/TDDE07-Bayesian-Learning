...
Xnames <- colnames(X)
if (standardize){
  Index <- 2:(length(Covs)-1) #normalize only index 2 to 10 of the data (if covs is columns 1-11)
...
Sigma <- (1/lambda)*diag(Npar) # Prior covariance matrix , 11x11 when 11 covs- diag(Npar) - diagonal matrix with 1s on the diagonal
...
LogPostLogistic <- function(betas,y,X,mu,Sigma){
  ...
  logPrior <- dmvnorm(betas, mu, Sigma, log=TRUE); #dmvnorm - Multivariate Normal Density and Random Deviates
...
initVal <- matrix(0,Npar,1) #default just 11 zeros, if 11 covs.
...
#solve gives inverse, i.e. a %*% x = b, solves for x. diag are the variances? 
